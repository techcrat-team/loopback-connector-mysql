...
function mixinMigration(MySQL, mysql) {
  ...
  /**
    ...
    * @param {Function} [cb] The callback function
    */
  MySQL.prototype.createForeignKeys = function (models, cb) {
    var self = this;
  ...
  MySQL.prototype.addIndexes = function (model, actualIndexes) {
    ...
    aiNames.forEach(function (indexName) {
      ...
      if ((indexNames.indexOf(indexName) === -1 && !m.properties[indexName]) &&
        ...
        (!m.settings.foreignKeys || !m.settings.foreignKeys[indexName])) {
  ...
  function stringOptionsByType(p, columnType) {
    switch (columnType.toLowerCase()) {
      ...
      case 'varchar':
        ...
        var len = (p.mysql && p.mysql.dataLength) || p.length || p.limit ||
          ((p.type !== String) ? 4096 : p.id || p.index ? 255 : 512);
        columnType += '(' + len + ')';
        break;
      case 'char':
        len = (p.mysql && p.mysql.dataLength) || p.length || p.limit || 255;
        columnType += '(' + len + ')';
        break;
  ...
  function floatingPointOptions(p, columnType) {
    var precision = 16;
    var scale = 8;
    if ((p.mysql && p.mysql.dataPrecision) || p.precision) {
      precision = Number((p.mysql && p.mysql.dataPrecision) || p.precision);
    }
    if ((p.mysql && p.mysql.dataScale) || p.scale) {
      scale = Number((p.mysql && p.mysql.dataScale) || p.scale);
    }
    if (((p.mysql && p.mysql.dataPrecision) || p.precision) && ((p.mysql && p.mysql.dataScale) || p.scale)) {
      columnType += '(' + ((p.mysql && p.mysql.dataPrecision) || p.precision) + ',' + ((p.mysql && p.mysql.dataScale) || p.scale) + ')';
    } else if (((p.mysql && p.mysql.dataPrecision) || p.precision)) {
      columnType += '(' + ((p.mysql && p.mysql.dataPrecision) || p.precision) + ')';
    }
    return columnType;
  }
  ...
  function integerOptions(p, columnType) {
    var tmp = 0;
    if ((p.mysql && p.mysql.dataLength) || p.length || p.limit) {
      tmp = Number((p.mysql && p.mysql.dataLength) || p.length || p.limit);
    }
    if (tmp > 0) {
      columnType += '(' + tmp + ')';
    } else if (p.unsigned) {
